...
week1_sleep[3] # Assuming week started on Sunday, thus taking 3rd index for Tuesday

## Create a vector called `week1_sleep_weekdays`
## Assign the weekday values using indice slicing
## Assuming week started on Sunday
...
# using slicing to exclude the index range.
# Everything ecxept mentioned index range will be included
weekends <- days[-2:-6]
weekends
...
# step 1 - determine days having more that 8 hours of sleep
week1_sleep > 8
# step 2 - select only those days / elements from the week1_sleep vector
week1_sleep[week1_sleep > 8]
# step 3 - use length() to determine number of days
...
# creating students_combined vector
students_combined <- c(student01, student02, student03)
students_combined
# creating grades matrix byrow with nrow = 3. One row each for each student. 
grades <- matrix(students_combined, byrow = TRUE, nrow = 3)
grades
student_num <- c("student01","student02","student03")
rownames(grades) <- student_num
assignment_num <- c("assignment01", "assignment02")
colnames(grades) <- assignment_num
grades
